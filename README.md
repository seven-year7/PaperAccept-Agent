...
- `POST /api/chat`、`POST /api/chat_stream`、`POST /api/chat/clear`、`GET /api/chat/session/{session_id}`
- `POST /api/upload`、`/api/upload/select`、`/api/upload/load`、`POST /api/index_directory`
- `POST /api/paper/research_stream`、`POST /api/paper/confirm_search`
- `GET /health`

| 位置 | 说明 |
|------|------|
| `README.md` | 本文件 |
| `docs/rag_hybrid_recall_logging.md` | 混合检索日志与排查 |
| `scripts/dump_rag_recall.py` | 导出召回全文到指定 Markdown |
| `aiops-docs/` | 示例知识文档 |
...
- **绝对/相对/平均门控**：基于 Milvus L2 距离与 `RAG_RETRIEVE_*` 阈值；未通过时返回「库中无可靠依据」类提示并约束模型勿编造。
- **观测**：`[INFO][RAG_OBS]` 含 `request_id`、`retrieve_called`、`chunk_count`、`distances`、`below_threshold`、`used_tool_output` 等。

---

# 附录 F：RAG 超时与降级

- **工具外层超时**：`RAG_RETRIEVE_TOOL_TIMEOUT_MS`；**检索短超时**：`RAG_MILVUS_SEARCH_TIMEOUT_MS`。
- 超时或异常时返回 `degraded=true` 与 `reason`/`stage`，主回答可继续，避免假称检索成功。

---

# 附录 G：长对话摘要机制

- 每 `RAG_SUMMARY_TRIGGER_ROUNDS` 轮（默认 10）可触发摘要更新；配置 `RAG_SUMMARY_*`。
- Redis 中 `conversation_summary`、`round_count` 等字段与 `[INFO][RAG_OBS]` 摘要字段对齐。

---

**版本**: v1.2.1（与 `pyproject.toml` 对齐）
