    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            if rows is None:
                ...
                raise ValueError(
                    f"embeddings 响应 data 为 None（OpenRouter 等可能返回 provider 错误）: {err!r}"
                )
            if len(rows) != len(texts):
                raise ValueError(
                    f"embeddings 返回条数 {len(rows)} 与输入 {len(texts)} 不一致"
                )
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        try:
            ...
            if rows is None or len(rows) < 1:
                err = getattr(response, "error", None)
                raise ValueError(
                    f"embeddings 响应 data 异常（None 或空，易原为 'NoneType' 下标错误）: "
                    f"data={rows!r} provider_error={err!r}"
                )
